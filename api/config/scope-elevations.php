...
return [
    ...
    'App\Http\Controllers\EnrolmentBatchController::enrolmentStatuses' => 'S04E STEP 3 dashboard enrichment (Spec Part H H4): reads the LIVE status of this batch\'s own enrolments (status enum only, no consent content) so the school sees how far each bulk-enrolled child has progressed. The enrolments are outside the school admin\'s derived scope; the read is confined to this batch\'s enrolment ids.',
    ...
    'App\Services\Programmes\WizardService::saveSection' => 'Programme capacity edit (S05-2): the seat counter is a system-only table (claimed moves only through Team Formation); this raises/lowers the CAPACITY column after the OD-31 lower-below-claimed guard, never claimed. Config authority was established by the wizard route before this call.',
    ...
    'App\Services\Teams\TeamTeacherLinkService::unlink' => 'Team-teacher UNLINK (S-MENTOR-1, OD-61): the lobby school admin or an academy admin removes a teacher\'s link to a TEAM; the mentor then loses the per-programme config-gated team view. team_teacher_links is a system-only write; the admin authority was established before the elevation.',
    ...
    // A-2 — the AuthorityGrantService is the ONLY writer of the delegation grant tables, and RLS makes those
