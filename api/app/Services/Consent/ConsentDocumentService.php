...
class ConsentDocumentService
{
    ...
    public function certificateHtml(object $signature, object $request, object $version, object $signer): string
    {
        ...
        return <<<HTML
            ...
            Self-contained PDF, all fonts embedded · Academy-issued; no co-branding, no external signatories.</p>
            HTML;
    }
    ...
    public function renderPdf(string $documentHtml, string $certificateHtml, string $language): string
    {
        ...
        $mpdf = new Mpdf([
            ...
            'format' => 'A4',
            'tempDir' => $tempDir,
            // NO PDF/A claim (Leo, S03-3 review, option b): mPDF's PDF/A output
            // fails veraPDF clause 6.3.5-3 (no CIDSet for subset fonts), and a
            // document whose own XMP claims a standard it fails is a cheap
            // attack on the whole evidence bundle. The file stays fully
            // self-contained (fonts embedded, no external references); real
            // PDF/A conformance is the S10 decision (AUDIT §5 item 5).
