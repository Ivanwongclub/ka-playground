...
| OD-22 | **Member surfaces unassigned** (new, raised at S01 card adjustment) | S01 seeds only | **OPEN — sprint assignment needed** | FR058 (Member: event list, RSVP, directory — OD-1) is not on any sprint card. S01 seeds the role and its denials but builds none of its purpose; a Member who logs in today lands on an empty shell. **Proposed: S06** — the Learn sprint builds the session/event machinery (lessons, events, quota, booking) that the Member event list and RSVP naturally sit on; the directory is a small read-only addition. Do NOT build in S01. Decision + card edit required before S06 starts |
