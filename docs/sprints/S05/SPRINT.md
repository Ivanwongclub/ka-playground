...
2. **Consent re-verified INSIDE the transaction under `SELECT … FOR SHARE` on each member's
   ...
   (or 成團 waits, re-reads, and refuses on stale — OD-57/58). **LOCK ORDER — STATED, BINDING (Leo, 2026-07-27): within 成團, lock consent_request
   rows FIRST (FOR SHARE, ordered by request id), THEN the programme capacity counter
   (FOR UPDATE), and release together at commit. Every future edit to this transaction
   preserves that order — it is the only guard against reintroducing the deadlock. Any
   step that needs both locks acquires them in this order, no exceptions.**
