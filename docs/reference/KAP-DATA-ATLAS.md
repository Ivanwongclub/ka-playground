...
| **fee_items** | trilingual, versioned at publish | snapshot into order_lines. `SUM(amount_minor)` is served to the **authenticated family** via the marketplace read under ONE `asSystem` elevation per REQUEST (never per programme — one audit row per authed storefront load; fee ruling, `f0004d1`). `fee_items_read` itself is UNCHANGED (still `system OR finance…`), and the ANONYMOUS payload carries no money field at all — which is what keeps `payment_links.single_reader` true in fact and not merely green. Zero fee items ⇒ the field is ABSENT, never `0` |
...
| **consent_requests** | status default `sent`; full set: draft·sent·viewed·signed·declined·expired·**superseded**·**voided** (`2026_07_25_150000`, used by `POST /admin/consent-requests/{id}/void`) · merge_data jsonb · **event_sequence jsonb** (the timeline) · **no `kind`** 🔴 P-1 (media consent) · **`expires_at` now written at EVERY issuance** (S-TTL-1; `min(issue+14d, starts_at)`, clamped only for a still-future start) and **`expired` — the one dead value in an otherwise live enum — now has its writer**, the nightly CAS sweeper. The three signature display facts (`signed_at · language · version`) ride the family LIST read via a 1:0..1 LEFT JOIN off `consent_signatures` (`request_id` UNIQUE ⇒ cannot fan out), each still gated by `cs_read`, which admits the signer alone among portal roles — a student or chasing school admin gets the row with all three NULL | never batched (OD-50); fresh per cohort (OD-53); completeness gates Formation (OD-51) |
...
| **team_members** | status (`active`, ≠removed) · category_id denorm · **programme_id ✅ (P-HYGIENE-1, `c56e5d4`)** — NOT NULL, backfilled from `teams` under system context, composite FK `(team_id, programme_id) → teams(id, programme_id)` (`tm_team_programme_fk`) making divergence unrepresentable | `tm_read`: own row + children + school-admin-of-lobby + mentor arm; roster to members via **allowlisted elevation** (names+role+count; contacts WITHHELD); formed-team write exclusivity = service-layer (F-5 carve-out 🔴) |
...
| **stage_gates** | 5 fixed stages · status default `passed` (passes only — absence = pending) · approver_kind · approved_at · **notes withheld from family read** · category_id denorm · **programme_id ✅ (P-HYGIENE-1, `c56e5d4`)** — composite FK `sg_team_programme_fk`; this is `stage_gates`' FIRST referential integrity, and it is what makes the mentor arm character-identical to `teams_read`'s on all three reads | tracker read widened S-TRACKER-1 (`passed_at`+`approver_kind`; identity+notes never selected); **no sequence lock** (D-7) |
